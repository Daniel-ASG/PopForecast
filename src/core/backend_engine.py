...
class PopForecastInferenceBackend:
    ...
    def _triangulate_rb_artist_id_batch(self, artist_name: str) -> str:
        """Backward-compatible wrapper for MB-to-RB artist triangulation."""
        return triangulate_rb_artist_id_batch(
            artist_name=artist_name,
            mb_headers=self.mb_headers,
            rb_url=self.rb_url,
            rb_headers=self.rb_headers,
            request_json=self._request_json,
            artist_name_match_score=self._artist_name_match_score,
            log_timed=self._log_timed,
        )
    ...
    def _resolve_inference_by_rb_track_id(
        ...
        Backward-compatible wrapper around the extracted ReccoBeats track inference
        helper.
        """
        return resolve_inference_by_rb_track_id(
            track_id=track_id,
            rb_url=self.rb_url,
            rb_headers=self.rb_headers,
            request_json=self._request_json,
            default_audio_features=DEFAULT_AUDIO_FEATURES,
            context_artist_id=context_artist_id,
        )
    ...
    def _harvest_rb_track_variants_from_catalog(
        ...
        """Backward-compatible wrapper for catalog-based curator variant harvesting."""
        return harvest_rb_track_variants_from_catalog(
            artist_id=artist_id,
            track_title=track_title,
            get_artist_catalog=self.get_rb_artist_catalog,
            get_album_tracks=self.get_rb_album_tracks,
            ...
            rb_url=self.rb_url,
            rb_headers=self.rb_headers,
            normalize_track_variant_title=self._normalize_track_variant_title,
            infer_contextual_track_type=self._infer_contextual_track_type,
            score_catalog_album_canonicality=self._score_catalog_album_canonicality,
            build_variant_representative_key=self._build_variant_representative_key,
